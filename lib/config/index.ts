...
export default async (rootDir: string): Promise<VueFrontConfig> => {
  ...
  if (!fs.existsSync(rootDir + '/node_modules/vuefront')) {
    throw new Error('No found VueFront');
  }
  const vuefrontDefaultConfig = await import(rootDir + '/node_modules/vuefront')
  ...
  console.log(rootDir + '/vuefront.config')
  if (!fs.existsSync(rootDir + '/vuefront.config')) {
    throw new Error('No found VueFront Config');
  }
  const themeConfig = await import(rootDir + '/vuefront.config')
  ...
  if (typeof config.app !== 'undefined') {
    for(const key in config.app) {
      ...
      let customAppConfig = await import(config.app[key])
  ...
  if (typeof config.theme !== 'undefined') {
    ...
    let customThemeConfig = await import(config.theme)
